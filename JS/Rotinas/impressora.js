...
const impressorasheetURL = `https://docs.google.com/spreadsheets/d/1UEpGrd5FfYzVe04pwBFN0BJqaZ6gJiioXGbp7vJyL-U/edit?usp=drive_link`;

// Função para carregar os dados da planilha na tabela com o id "data-table"
function loadimpressoraSheetData(url) {
    fetch(url)
        ...
        .then(data => {
            ...
            const dataTableBody = document.querySelector('#data-table-impressora tbody');
            ...
            for (let i = 2; i < maxRows; i++) {
                ...
                // Verificar se todas as colunas relevantes (ID_impressora, ERROS, FOTOS, SOLUCOES) estão preenchidas
                if (columns[0].textContent.trim() !== '') {

                    const errorsressumoimpressoraCell = document.createElement('td');
                    const errorsressumoimpressoraData = columns[2].textContent.trim();
                    errorsressumoimpressoraCell.innerHTML = `<span>${errorsressumoimpressoraData}</span>`;

                    errorsressumoimpressoraCell.classList.add('erro1'); // Substitua 'sua-classe-aqui' pelo nome da classe desejada
                    ...
                    viewButton.addEventListener('click', () => {
                        ...
                        /*                         // Selecionando os inputs pelos IDs
                                                const errosInput = document.getElementById('P_impressora-edit');
                                                const solucoesInput = document.getElementById('P_impressora1-edit');
                                                const scriptInput = document.getElementById('P_impressora2-edit');
                        ...
                        const linksString = linksArray.map((link, index) => `
                            <a href="${link}" id="P_impressora${index + 1}" target="_blank" class="buttonDownload">${descricaoAnexoArray[index]}</a>
                        `).join('');
                        ...
                        <div class="imagem-div" >
                            <img class="img_anexo imagem-div1" id="P_impressora4${index + 1}" src="${link}" alt="">
                        </div>
                        `).join('');
                        ...
                        modalContent.innerHTML = `
                            <div class="modal1">
                                <h3>${sistema}</h3>
                                <a id="EDITFORMSHEET-impressora" title="Editar!"><img src="IMG/EDIT.png" class="EDIT-FORM-SHEET"></a>
                            </div>
                            <div id="column">
                                <div class="modal2" id="modal2" class="scroll-modal">
                                    <h3>Erro Detalhado: </h3>
                                    <hr class="hr-rot">
                                    <p id="P_impressora">${erros}</p>
                                </div>
                                <div class="modal3" class="scroll-modal">
                                    <h3>Soluções: </h3>
                                    <hr class="hr-rot">
                                    <p id="P_impressora1">${solucoes}</p>
                                </div>
                            </div>
                            <div id="column">
                                <div class="modal4" style="${script ? '' : 'display: none;'}">
                                    <h3>Scripts:</h3>
                                    <a href="#" title="Copiar!" id="copyLink" onclick="copyToClipboardscriptimpressora()" >📄</a>
                                    <hr class="hr-rot">
                                    <p id="P_impressora2" class="scroll-modal">${script}</p>
                                </div>
                        ...
                        const openEditFormButton = document.getElementById('EDITFORMSHEET-impressora');
                        ...
                        transformarEmListaOrdenada('P_impressora'); // Substituir 'P_impressora1' pelo ID desejado
                        transformarEmListaOrdenada('P_impressora1'); // Substituir 'P_impressora2' pelo ID desejado                        
                        ...
                        formatarTextoComQuebrasDeLinha('P_impressora');
                        formatarTextoComQuebrasDeLinha('P_impressora1');
                        formatarTextoComQuebrasDeLinha2('P_impressora2');
                        formatarTextoComQuebrasDeLinha2('P_impressora3');
                        formatarTextoComQuebrasDeLinha2('P_impressora4');
                        formatarTextoComQuebrasDeLinha2('P_impressora5');

                        //quebrar linha do edit
                        formatarTextoComQuebrasDeLinhaEDIT('P_impressora_edit');
                        formatarTextoComQuebrasDeLinhaEDIT('P_impressora1_edit');
                        formatarTextoComQuebrasDeLinhaEDIT2('P_impressora2_edit');
                        formatarTextoComQuebrasDeLinhaEDIT2('P_impressora3_edit');
                        formatarTextoComQuebrasDeLinhaEDIT2('P_impressora4_edit');
                        formatarTextoComQuebrasDeLinhaEDIT2('P_impressora5_edit');
                    });

                    viewButtonCell.appendChild(viewButton); // Adicionar o botão à célula

                    const newRow = document.createElement('tr');
                    newRow.appendChild(errorsressumoimpressoraCell);
...
const closeModalimpressoraButton = document.getElementById('closeModal');
const infoModalimpressora = document.getElementById('infoModal');

closeModalimpressoraButton.addEventListener('click', () => {
    infoModalimpressora.style.display = 'none';
});


// Chamar a função para carregar os dados da planilha
loadimpressoraSheetData(impressorasheetURL);

/* FIM DADOS */

/* Copiar Script-impressora*/
function copyToClipboardscriptimpressora() {
    const infoText = document.getElementById("P_impressora2").innerText;
