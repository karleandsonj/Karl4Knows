...
const tefsheetURL = `https://docs.google.com/spreadsheets/d/1xgy7EN6j7DhBRjFLtzVdG3SoSa_qCOKzHIvDBln72qQ/edit?usp=drive_link`;

// Função para carregar os dados da planilha na tabela com o id "data-table"
function loadtefSheetData(url) {
    fetch(url)
        ...
        .then(data => {
            ...
            const dataTableBody = document.querySelector('#data-table-tef tbody');
            ...
            for (let i = 2; i < maxRows; i++) {
                ...
                // Verificar se todas as colunas relevantes (ID_tef, ERROS, FOTOS, SOLUCOES) estão preenchidas
                if (columns[0].textContent.trim() !== '') {

                    const errorsressumotefCell = document.createElement('td');
                    const errorsressumotefData = columns[2].textContent.trim();
                    errorsressumotefCell.innerHTML = `<span>${errorsressumotefData}</span>`;

                    errorsressumotefCell.classList.add('erro1'); // Substitua 'sua-classe-aqui' pelo nome da classe desejada
                    ...
                    viewButton.addEventListener('click', () => {
                        ...
                        /*                         // Selecionando os inputs pelos IDs
                                                const errosInput = document.getElementById('P_tef-edit');
                                                const solucoesInput = document.getElementById('P_tef1-edit');
                                                const scriptInput = document.getElementById('P_tef2-edit');
                        ...
                        const linksString = linksArray.map((link, index) => `
                            <a href="${link}" id="P_tef${index + 1}" target="_blank" class="buttonDownload">${descricaoAnexoArray[index]}</a>
                        `).join('');
                        ...
                        <div class="imagem-div" >
                            <img class="img_anexo imagem-div1" id="P_tef4${index + 1}" src="${link}" alt="">
                        </div>
                        `).join('');
                        ...
                        modalContent.innerHTML = `
                            <div class="modal1">
                                <h3>${sistema}</h3>
                                <a id="EDITFORMSHEET-tef" title="Editar!"><img src="IMG/EDIT.png" class="EDIT-FORM-SHEET"></a>
                            </div>
                            <div id="column">
                                ...
                                <div class="modal3" class="scroll-modal">
                                    <h3>Soluções: </h3>
                                    <hr class="hr-rot">
                                    <p id="P_tef1">${solucoes}</p>
                                </div>
                            </div>
                            <div id="column">
                                <div class="modal4" style="${script ? '' : 'display: none;'}">
                                    <h3>Scripts:</h3>
                                    <a href="#" title="Copiar!" id="copyLink" onclick="copyToClipboardscripttef()" >📄</a>
                                    <hr class="hr-rot">
                                    <p id="P_tef2" class="scroll-modal">${script}</p>
                                </div>
                        ...
                        const openEditFormButton = document.getElementById('EDITFORMSHEET-tef');
                        ...
                        EditForm.innerHTML = `
                                <input type="hidden" id="uniqueID_tef_Edit" name="uniqueID" style="display: none;">
                                <input type="hidden" id="sistema_tef_EDIT" name="sistema" style="display: none;">
                                <input type="hidden" id="img_tef_EDIT" name="imageUrl" style="display: none;">
                                <input type="hidden" id="nomearq_tef_EDIT" name="descricao_anexo" style="display: none;">
                                <input type="hidden" id="arq_tef_EDIT" name="anexoUrl" style="display: none;">
                                ...
                                <div class="ErroSolucaoFORM-EDIT" id="ErroSolucaoFORM-EDIT">
                                    <div class="modal1-edit" id="modal1-edit">
                                        <h3>Erro Resumido: </h3>
                                        <input type="text" id="P_tef0_edit" name="erro_resumido" placeholder="Erro Resumido" class="campo0-edit" autocomplete="off">
                                    </div>
                                    <div class="modal2-edit" id="modal2-edit">
                                        <h3>Erro Detalhado: </h3>
                                        <textarea type="text" id="P_tef_edit" name="erro" placeholder="Erros" class="campo-edit" autocomplete="off"></textarea>
                                    </div>
                                    <div class="modal3-edit">
                                        <h3>Soluções: </h3>
                                        <textarea type="text" id="P_tef1_edit" name="solucao" placeholder="Soluções" class="campo-edit" autocomplete="off"></textarea>
                                    </div>
                                </div>
                                <div class="AnexoScriptFORM-EDIT" id="AnexoScriptFORM-EDIT" style="display: none;">
                                    <div class="modal4-edit">
                                        <h3>Scripts:</h3>
                                        <textarea type="text" id="P_tef2_edit" name="script" placeholder="Script" class="campo-edit" autocomplete="off"></textarea>
                                    </div>
                                </div>
                        ...
                        submitButton.addEventListener('click', e => {
                            const textareaedit1 = document.getElementById('P_tef_edit');
                            const textareaedit2 = document.getElementById('P_tef1_edit');
                        ...
                        function sendFormEdit() {
                            ...
                            const uniqueIDEdit = document.getElementById('uniqueID_tef_Edit').value;
                        ...
                        const uniqueID_tef_Edit = document.getElementById("uniqueID_tef_Edit");
                        const sistema_tef_edit = document.getElementById("sistema_tef_EDIT");
                        const P_tef0_edit = document.getElementById("P_tef0_edit");
                        const P_tef_edit = document.getElementById("P_tef_edit");
                        const P_tef1_edit = document.getElementById("P_tef1_edit");
                        const P_tef2_edit = document.getElementById("P_tef2_edit");
                        const img_tef_EDIT = document.getElementById("img_tef_EDIT");
                        const nomearq_tef_EDIT = document.getElementById("nomearq_tef_EDIT");
                        const arq_tef_EDIT = document.getElementById("arq_tef_EDIT");

                        img_tef_EDIT.value = imagemLink;
                        nomearq_tef_EDIT.value = descricao_anexo;
                        arq_tef_EDIT.value = anexo;
                        P_tef0_edit.value = errorsressumotefData;
                        P_tef_edit.value = erros;
                        P_tef1_edit.value = solucoes;
                        P_tef2_edit.value = script;
                        sistema_tef_edit.value = sistema;
                        uniqueID_tef_Edit.value = uniqueID;
                        ...
                        transformarEmListaOrdenada('P_tef'); // Substituir 'P_tef1' pelo ID desejado
                        transformarEmListaOrdenada('P_tef1'); // Substituir 'P_tef2' pelo ID desejado                        
                        ...
                        formatarTextoComQuebrasDeLinha('P_tef');
                        formatarTextoComQuebrasDeLinha('P_tef1');
                        formatarTextoComQuebrasDeLinha2('P_tef2');
                        formatarTextoComQuebrasDeLinha2('P_tef3');
                        formatarTextoComQuebrasDeLinha2('P_tef4');
                        formatarTextoComQuebrasDeLinha2('P_tef5');

                        //quebrar linha do edit
                        formatarTextoComQuebrasDeLinhaEDIT('P_tef_edit');
                        formatarTextoComQuebrasDeLinhaEDIT('P_tef1_edit');
                        formatarTextoComQuebrasDeLinhaEDIT2('P_tef2_edit');
                        formatarTextoComQuebrasDeLinhaEDIT2('P_tef3_edit');
                        formatarTextoComQuebrasDeLinhaEDIT2('P_tef4_edit');
                        formatarTextoComQuebrasDeLinhaEDIT2('P_tef5_edit');
                    });

                    viewButtonCell.appendChild(viewButton); // Adicionar o botão à célula

                    const newRow = document.createElement('tr');
                    newRow.appendChild(errorsressumotefCell);
...
const closeModaltefButton = document.getElementById('closeModal');
const infoModaltef = document.getElementById('infoModal');

closeModaltefButton.addEventListener('click', () => {
    infoModaltef.style.display = 'none';
});


// Chamar a função para carregar os dados da planilha
loadtefSheetData(tefsheetURL);

/* FIM DADOS */

/* Copiar Script-tef*/
function copyToClipboardscripttef() {
    const infoText = document.getElementById("P_tef2").innerText;
