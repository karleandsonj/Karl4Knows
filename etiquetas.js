...
const etiquetassheetURL = `https://docs.google.com/spreadsheets/d/1Ehel5zPM8sINk7Crv5sZK_ubA1zQEv4wOn-BOPZxkFM/edit?usp=drive_link`;

// Função para carregar os dados da planilha na tabela com o id "data-table"
function loadetiquetasSheetData(url) {
    fetch(url)
        ...
        .then(data => {
            ...
            const dataTableBody = document.querySelector('#data-table-etiquetas tbody');
            ...
            for (let i = 2; i < maxRows; i++) {
                ...
                // Verificar se todas as colunas relevantes (ID_etiquetas, ERROS, FOTOS, SOLUCOES) estão preenchidas
                if (columns[0].textContent.trim() !== '') {

                    const errorsressumoetiquetasCell = document.createElement('td');
                    const errorsressumoetiquetasData = columns[2].textContent.trim();
                    errorsressumoetiquetasCell.innerHTML = `<span>${errorsressumoetiquetasData}</span>`;

                    errorsressumoetiquetasCell.classList.add('erro1'); // Substitua 'sua-classe-aqui' pelo nome da classe desejada
                    ...
                    viewButton.addEventListener('click', () => {
                        ...
                        /*                         // Selecionando os inputs pelos IDs
                                                const errosInput = document.getElementById('P_etiquetas-edit');
                                                const solucoesInput = document.getElementById('P_etiquetas1-edit');
                                                const scriptInput = document.getElementById('P_etiquetas2-edit');
                        ...
                        const linksString = linksArray.map((link, index) => `
                            <a href="${link}" id="P_etiquetas${index + 1}" target="_blank" class="buttonDownload">${descricaoAnexoArray[index]}</a>
                        `).join('');
                        ...
                        <div class="imagem-div" >
                            <img class="img_anexo imagem-div1" id="P_etiquetas4${index + 1}" src="${link}" alt="">
                        </div>
                        `).join('');
                        ...
                        modalContent.innerHTML = `
                            <div class="modal1">
                                <h3>${sistema}</h3>
                                <a id="EDITFORMSHEET-etiquetas" title="Editar!"><img src="IMG/EDIT.png" class="EDIT-FORM-SHEET"></a>
                            </div>
                            <div id="column">
                                <div class="modal2" id="modal2" class="scroll-modal">
                                    <h3>Erro Detalhado: </h3>
                                    <hr class="hr-rot">
                                    <p id="P_etiquetas">${erros}</p>
                                </div>
                                <div class="modal3" class="scroll-modal">
                                    <h3>Soluções: </h3>
                                    <hr class="hr-rot">
                                    <p id="P_etiquetas1">${solucoes}</p>
                                </div>
                            </div>
                            <div id="column">
                                <div class="modal4" style="${script ? '' : 'display: none;'}">
                                    <h3>Scripts:</h3>
                                    <a href="#" title="Copiar!" id="copyLink" onclick="copyToClipboardscriptetiquetas()" >📄</a>
                                    <hr class="hr-rot">
                                    <p id="P_etiquetas2" class="scroll-modal">${script}</p>
                                </div>
                        ...
                        const openEditFormButton = document.getElementById('EDITFORMSHEET-etiquetas');
                        ...
                        transformarEmListaOrdenada('P_etiquetas'); // Substituir 'P_etiquetas1' pelo ID desejado
                        transformarEmListaOrdenada('P_etiquetas1'); // Substituir 'P_etiquetas2' pelo ID desejado                        
                        ...
                        formatarTextoComQuebrasDeLinha('P_etiquetas');
                        formatarTextoComQuebrasDeLinha('P_etiquetas1');
                        formatarTextoComQuebrasDeLinha2('P_etiquetas2');
                        formatarTextoComQuebrasDeLinha2('P_etiquetas3');
                        formatarTextoComQuebrasDeLinha2('P_etiquetas4');
                        formatarTextoComQuebrasDeLinha2('P_etiquetas5');

                        //quebrar linha do edit
                        formatarTextoComQuebrasDeLinhaEDIT('P_etiquetas_edit');
                        formatarTextoComQuebrasDeLinhaEDIT('P_etiquetas1_edit');
                        formatarTextoComQuebrasDeLinhaEDIT2('P_etiquetas2_edit');
                        formatarTextoComQuebrasDeLinhaEDIT2('P_etiquetas3_edit');
                        formatarTextoComQuebrasDeLinhaEDIT2('P_etiquetas4_edit');
                        formatarTextoComQuebrasDeLinhaEDIT2('P_etiquetas5_edit');
                    });

                    viewButtonCell.appendChild(viewButton); // Adicionar o botão à célula

                    const newRow = document.createElement('tr');
                    newRow.appendChild(errorsressumoetiquetasCell);
...
const closeModaletiquetasButton = document.getElementById('closeModal');
const infoModaletiquetas = document.getElementById('infoModal');

closeModaletiquetasButton.addEventListener('click', () => {
    infoModaletiquetas.style.display = 'none';
});


// Chamar a função para carregar os dados da planilha
loadetiquetasSheetData(etiquetassheetURL);

/* FIM DADOS */

/* Copiar Script-etiquetas*/
function copyToClipboardscriptetiquetas() {
    const infoText = document.getElementById("P_etiquetas2").innerText;
